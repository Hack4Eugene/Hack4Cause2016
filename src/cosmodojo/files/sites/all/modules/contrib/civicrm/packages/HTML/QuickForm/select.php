...
class HTML_QuickForm_select extends HTML_QuickForm_element {
    ...
    function toHtml()
    {
        if ($this->_flagFrozen) {
            ...
            // PATCHED TO FIX ISSUE with array structure of values to be used
            // Cord Slatton-Valle - 02/06/16
            if(is_array($this->_values) && isset($this->_values[0]) && is_array($this->_values[0])){
                $strValues = array_map('strval', $this->_values[0]);
            }
            else if(is_array($this->_values)){
                $strValues = array_map('strval', $this->_values);
            }
            else{
                $strValues = array();
            }
